...
또한 `SELECT` 요청은 큐를 타지 않기 때문에,
만약 조회 쿼리 자체가 특정 리소스에 집중되는 경우
(예: 인기 상품 조회 API)에는 별도의 캐싱 전략이나 샤딩이 필요하다.
...
원하면 여기에 **아키텍처 다이어그램 (Tomcat → Queue → Worker → DB)** 도 같이 넣는 버전도 만들어줄게.
그림까지 들어가면 README 완성도 확 올라가. 넣을까?
