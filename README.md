...
Db락으로 인해 나머지 톰캣스레드들이 모두 db커넥션을 반납하지못하고 대기중 ,이때 새로운api요청시 가용가능한 톰캣스레드가 응답(카운터)을 받으러 갈수없다 "동기"- >
기존 톰캣스레드의 역할(카운터직원+주방요리사)을 분리해야 하지 않을까? ->
      카운터직원(사용자에게 선착순이벤트 페이지를 보여주는 로직)
      주방요리사(db insert 처리 로직)

-> 큐라는 자료구조(+워커스레드)를 이용해 분리한다.
      톰캣스레드는 카운터직원역할만 수행
      더이상 톰캣스레드가 db커넥션을 반납하지못하고 대기하는 상황은 존재하지않음.
      insert 손님이 오면 , 바로 큐에 주문서를 꽂고 사용자에게 응답처리한다.
      워커스레드 1개가 큐 주문서 오면-db커넥션 획득-db insert 처리 - db 커넥션 반납 과정을 반복한다.
